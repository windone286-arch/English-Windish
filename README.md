# English-Windish

> 拍照 → 识别 → 语法解析 → 单词精讲。一个为英语学习者（尤其是初高中学生）打造的 AI 辅助学习工具。

**🔗 在线试用：https://english-windish.app.workbuddy.host/**（设有访问密码，用于防止 API 额度被滥用，密码请向项目所有者索取）

## 这个项目解决什么问题

学生在做英语阅读理解、完形填空、试卷订正时，会遇到三类问题：

1. **看不懂句子结构**——固定搭配、复杂句型、特殊句式识别不出来，靠语感硬猜
2. **查单词效率低**——词典 App 只给中文释义，不给词根词缀、不给词源、不给搭配例句
3. **记录成本高**——错题、生词散落在试卷和笔记本上，无法系统复习

本项目把这三件事合成一个动作：**拍一张照片，得到一份结构化的学习报告**。

## 核心功能

**两种输入方式**：拍照/上传图片，或直接输入单词、词组、句子。
**三个结果分栏**：翻译 / 语法解析 / 单词精讲，三者是独立 Tab，可随时切换回看。

界面会**按输入粒度自动调整措辞**：输入单词或词组时，第一个 Tab 叫「释义」而不是「全文翻译」，
语法分栏则直接说明「没有句子结构可分析，请看单词精讲」。
一个词本来就没有全文可译，沿用一个句子的文案只会让人以为功能出了问题。

### 功能一：拍照语法解析

上传/拍摄一张英文文本图片，系统输出：

- **原文识别**：多模态大模型直接读图，无需本地 OCR
- **中文翻译**：逐句对照翻译
- **语法标注**：自动标出
  - 固定搭配（如 `be accustomed to doing`）
  - 复杂句型（如定语从句、状语从句、名词性从句）
  - 特殊句式（如倒装、强调句、虚拟语气、独立主格）
- **同步呈现**：语法标注与翻译在同一栏中逐句对齐

### 功能二：单词精讲

对文本中的重点词，输出结构化词卡：

| 板块 | 内容 |
|---|---|
| **词根词缀** | 拆解构词，说明每个词素含义 |
| **词源 / 核心原意** | 追溯词源，点出核心义（帮助理解引申义） |
| **中文释义** | 默认展示高频义项，可展开查看全部（按词性分组） |
| **固定搭配** | 常见高频搭配，每条配**英文例句 + 中文翻译** |

### 功能三：文本直接输入

没有图片时（比如只想查一个生词、分析一个长难句），直接在输入框敲进去即可。
后端走的是同一条分析流水线，只是一个跳过图片识别、直接进入语法与词汇解析。
上限 20000 字符，够放一篇短文。

**为什么必须要有这个功能？** 拍照链路的成本结构是「视觉模型 + 文本模型」两次调用，
查一个单词也要付两次钱、等十几秒。文本输入把视觉那一步省掉，日常查词才变得可用。

### 功能四：历史记录

每次分析完成自动落库（本地 SQLite），抽屉式列表按时间倒序展示，
点任意一条可还原完整结果，也能单条删除或清空。

**为什么存本地而不存云端？** 学生的错题和课文原文属于个人数据，
放本地既省了服务器成本，也避开了「用户内容上云」的合规麻烦。
代价是换设备不同步——这在当前阶段是划算的取舍。

## 界面预览

**输入**：支持拍照上传，也支持直接敲文本（查一个生词不必先拍照）。

![主界面](docs/screenshots/02-home.png)

**分析结果**：导航栏按「翻译 / 语法 / 单词」拆成三个独立页签，不用在一屏里上下翻找。
界面还会按输入粒度自动调整措辞——输入单词时第一个页签叫「释义」而不是「全文翻译」。

![结果页](docs/screenshots/04-result-translation.png)

**单词精讲**是本项目的重点：不只给中文释义，而是拆解构词、追溯词源、
列出高频搭配并逐条配例句与翻译。

![单词精讲](docs/screenshots/06-result-vocabulary.png)

更多截图见 `docs/screenshots/`。

## 技术架构

```
用户上传图片
     │
     ▼
┌─────────────────────┐
│  多模态大模型（视觉） │  ← 直接读图，规避传统 OCR 对手写体的识别瓶颈
└─────────────────────┘
     │  结构化文本
     ▼
┌─────────────────────┐
│  语法解析 Pipeline   │  ← Prompt 工程：先给依据，再给结论，降低幻觉
└─────────────────────┘
     │
     ├──────────────┬──────────────┐
     ▼              ▼              ▼
  翻译对齐       语法标注        知识点抽取
     │              │              │
     └──────────────┴──────────────┘
                    │
                    ▼
┌─────────────────────┐
│   单词精讲引擎       │  ← 词根词缀 / 词源 / 释义分级 / 搭配例句
└─────────────────────┘
     │
     ▼
┌─────────────────────┐
│  Web 界面（分栏展示）│
└─────────────────────┘
```

## 技术栈

| 层次 | 选型 | 说明 |
|---|---|---|
| 语言 | Python 3.13 | |
| 视觉模型 | 通义千问 `qwen-vl-max` | DeepSeek 无视觉模型，故图片识别走通义 |
| 文本模型 | DeepSeek `deepseek-chat` | 推理能力强，负责语法分析与单词精讲 |
| 后端框架 | FastAPI | 原生 async，适配「等待外部 API 数十秒」的场景 |
| 实时通信 | **SSE（Server-Sent Events）** | 分析耗时约 18 秒，需实时推送进度 |
| 前端 | 原生 HTML / CSS / JavaScript | 无框架依赖，零构建步骤 |
| 数据存储 | SQLite | 单词词卡缓存 + 历史记录（单文件、零配置） |
| 部署 | 待定 | |

### 为什么用 SSE 而不是 WebSocket

本场景是**单向推送**——客户端提交任务后，只有服务器往客户端推进度，
客户端不需要在分析过程中发消息。SSE 正好匹配：

- 基于 HTTP，无需额外的协议握手
- 浏览器原生 `EventSource` 支持
- 断线自动重连

WebSocket 是双向协议，用在这里属于过度设计。

### 为什么前端用 `fetch` 读流而不是 `EventSource`

`EventSource` 只支持 GET 请求，但上传文件必须用 POST。
所以前端用 `fetch` + `response.body.getReader()` 手动解析 SSE 流，
并自行处理分块边界（一个事件可能被切成两半）。

配套还有两个容易忽略的细节：解码必须用 `TextDecoder` 且带上 `{ stream: true }`，
否则一个中文字符（3 字节）被 chunk 边界切开就会解出乱码——这种问题只在特定网速下复现，最难排查；
以及必须用缓冲区累积字节，遇到空行才切出一个完整事件。

### 为什么进度阶段要分两套

图片链路 4 步（识别图片文字 → 分析语法 → 生成词卡 → 汇总），
文本链路 3 步（没有识别这一步）。

起初两处共用同一份阶段常量，结果文本分析时进度条第一步显示「识别图片文字」——
那一步根本不会执行。**进度条写错比没有进度条更糟**：
用户会盯着一个永远停在 0% 的步骤等下去。所以图片和文本各自维护一套阶段定义，
并用测试锁住「文本阶段 == 图片阶段去掉第一步」这个关系，防止将来改动时漂移。

## 部署

### 两道锁

部署到公网意味着任何拿到链接的人都能打开这个站点，
而每一次分析都在消耗**真实付费**的 API 额度，所以设了两道锁：

| 锁 | 环境变量 | 作用 |
|---|---|---|
| 访问密码 | `ACCESS_PASSWORD` | 拦住陌生人与爬虫 |
| 每日上限 | `DAILY_LIMIT` | 兜住最坏情况 |

**为什么需要两道？** 密码是可以被转发出去的，转发出去就收不回来。
每日上限保证「最坏情况下的损失」始终可估算：不管多少人拿到密码，
一天最多只消耗这么多额度。两个变量都留空时，两道锁都不生效。

### 密码是怎么校验的

不是把密码塞进 cookie 反复比对，而是：

1. 校验通过后，服务端签发一个 `过期时间.签名` 形式的凭证
2. 签名是对过期时间做的 HMAC，密钥**由密码本身派生**
3. 之后每个请求只需验签，不查内存也不查数据库

这样设计带来三个好处：

- **cookie 里不含密码** —— 即使凭证泄露，密码本身不泄露
- **改密码即吊销全部凭证** —— 密钥变了，所有旧签名立刻失效
- **服务重启不掉登录状态** —— 无状态，这对会休眠、会重启的免费托管平台很重要

### 部署要求

服务需要监听 `PORT` 环境变量并绑定 `0.0.0.0`。
代码已适配：检测到 `PORT` 就进入部署模式，否则退回本地 `127.0.0.1:8000`。

## 开发进度

- [x] 阶段 0：环境搭建、GitHub 仓库初始化
- [x] 阶段 1：核心功能（命令行版跑通全链路）
- [x] 阶段 2：Web 界面 + SSE 实时进度
- [x] 阶段 2.5：结果分栏 Tab、历史记录、文本直接输入
- [x] 阶段 2.6：交互细节修复（进度文案、页签文案、抽屉关不掉）
- [x] 阶段 3：访问保护（密码 + 每日限额）与线上部署
- [ ] 阶段 4：性能优化、文档完善

## 项目结构

```
English-Windish/
├── README.md              # 项目说明（本文件）
├── requirements.txt       # 运行时依赖（部署只装这个）
├── requirements-dev.txt   # 开发依赖（测试、数据处理等，部署不需要）
├── .gitignore             # Git 忽略规则
├── .env.example           # 环境变量模板（真实 key 不入库）
├── src/
│   ├── config.py          # 配置管理（双轨：视觉服务商 + 文本服务商）
│   ├── models.py          # 数据模型定义
│   ├── prompts.py         # Prompt 模板集中管理
│   ├── llm_client.py      # 大模型调用封装（多服务商 + JSON 容错 + 重试）
│   ├── pipeline.py        # 分析流水线（CLI 与 Web 共用）
│   ├── storage.py         # 历史记录 + 每日用量计数（SQLite）
│   ├── main.py            # 命令行入口
│   ├── ocr/               # 图像识别
│   ├── grammar/           # 语法解析
│   ├── vocabulary/        # 单词精讲（含 SQLite 缓存）
│   ├── report/            # 报告渲染（Markdown / JSON）
│   └── web/               # Web 应用
│       ├── app.py         # FastAPI 后端（SSE 流式进度 + 访问控制）
│       ├── auth.py        # 访问密码：签发与校验登录凭证
│       └── static/        # 前端（原生 HTML/CSS/JS）
├── data/
│   ├── samples/           # 示例图片
│   ├── uploads/           # 用户上传（.gitignore 排除）
│   ├── reports/           # 生成的分析报告
│   └── history.db         # 历史记录与用量库（运行时生成，.gitignore 排除）
├── docs/
│   └── devlog.md          # 开发日志（含决策记录与踩坑复盘）
└── tests/                 # 测试（120 个，不依赖真实 API）
```

## 快速开始

### 1. 环境准备

```bash
git clone git@github.com:windone286-arch/English-Windish.git
cd English-Windish

# 创建虚拟环境
python -m venv .venv

# 激活（Windows）
.venv\Scripts\activate

# 安装开发依赖（它已经包含运行时依赖，本地开发装这一个就够）
pip install -r requirements-dev.txt
```

> 部署时只装 `requirements.txt`。两个文件分开是为了让沙箱里的依赖安装更快、
> 失败率更低——测试框架和数据处理库在线上完全没有用武之地。

### 2. 配置 API Key

```bash
copy .env.example .env
```

编辑 `.env`，填入你的密钥：

```ini
# 视觉：负责图片识别（DeepSeek 没有视觉模型，必须用通义或智谱）
VISION_PROVIDER=dashscope
DASHSCOPE_API_KEY=sk-你的通义key

# 文本：负责语法分析与单词精讲
TEXT_PROVIDER=deepseek
DEEPSEEK_API_KEY=sk-你的deepseek key
```

> `.env` 已被 `.gitignore` 排除，不会被提交。

> **本地开发不用配 `ACCESS_PASSWORD`。** 它留空时访问保护完全关闭，
> 打开 http://127.0.0.1:8000 直接就能用。只有部署到公网时才需要设置，
> 原因见下方「部署」一节。

### 3. 运行

**Web 界面（推荐）**

```bash
python -m src.web.app
```

打开 http://127.0.0.1:8000

**命令行**

```bash
# 分析图片
python -m src.main data/samples/test1.jpg

# 指定词卡数量
python -m src.main data/samples/test1.jpg --words 10

# 只识别文字（省 API 额度，用于快速验证图片质量）
python -m src.main data/samples/test1.jpg --text-only

# 直接分析文本，跳过图片识别（调试 Prompt 用）
python -m src.main --text "The book which I bought yesterday is interesting."

# 输出 JSON
python -m src.main data/samples/test1.jpg --json
```

### 4. 运行测试

```bash
pytest tests/ -v
```

## 开发日志

开发过程记录见 [docs/devlog.md](docs/devlog.md)。

## License

MIT
