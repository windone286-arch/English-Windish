...
## 2026-09-19 · 阶段 3：访问保护与线上部署

### 背景

这个项目的验收标准之一是「能给面试官一个点开就能用的链接」。
但把链接放出去，意味着两件事同时发生：**任何人都能打开它**，
而**每一次分析都在消耗真实付费的 API 额度**。

所以部署这件事，技术难点不在「怎么传上去」，而在「怎么保证不被滥用」。
...
| 锁 | 环境变量 | 拦住什么 |
|---|---|---|
| 访问密码 | `ACCESS_PASSWORD` | 陌生人、爬虫、误点进来的人 |
| 每日上限 | `DAILY_LIMIT` | 密码被转发之后的滥用 |

关键认识：**密码是可以被转发出去的，而转发出去就收不回来**。
如果只有密码，那么「损失上限」等于「拿到密码的人数 × 无限次调用」，
不可估算。加上每日上限之后，最坏情况变成「一天最多 30 次分析」，
这个数字是可以算出来的——**可控的损失才叫风险，不可控的叫事故**。

**2. 密码怎么存：cookie 里放签名，不放密码**

最直觉的做法是把密码写进 cookie，每次请求比对。问题是：

- 一旦 cookie 泄露，泄露的是密码本身，而用户不可能天天改密码
- 想「登出所有设备」就得额外维护一套会话表

改成签发凭证：cookie 里放 `过期时间.HMAC签名`，而**签名密钥由密码派生**。
带来三个好处：

- cookie 不含密码，泄露了也只是泄露一个 30 天后过期的凭证
- 改密码 = 密钥变了 = 所有旧凭证立刻失效，天然支持「一键登出全部设备」
- 校验不查内存也不查数据库，**服务重启不掉登录**——
  这对会休眠、会重启的免费托管平台是刚需

**3. 依赖为什么要拆成两个文件**

部署时要在沙箱里现场装一遍依赖，**安装耗时和失败率都随包数量上升**。

核查后发现一个尴尬事实：`pandas`、`pillow`、`requests`、`rich`、`jinja2`
这五个包**一行代码都没被引用**——它们是立项时「以后可能用得上」预留的，
但阶段 1 到 3 全都没用上，却会在每次部署时被安装一遍。

一并处理了另一个隐患：`uvicorn[standard]` 会拉 `httptools` 和 `uvloop`，
这两个是**需要 C 编译器**的扩展包，在没有编译环境的沙箱里很容易装不上。
换成纯 Python 的 `uvicorn`——本项目的并发量根本用不到 uvloop 的性能优势。

### 踩的坑

**配了访问密码之后，测试全线失败**

现象：本地 97 个测试全绿。把 `ACCESS_PASSWORD` 写进 `.env` 之后，
**29 个测试突然失败**，报错是「页面里找不到抽屉元素」「页面结构不对」，
完全指不到真正的原因。

根因：`config.py` 在**导入时**用 `load_dotenv` 把 `.env` 读进了 `os.environ`，
于是测试进程里也「启用了访问保护」。所有测试请求被中间件重定向到登录页或返回 401，
测试拿到的其实是登录页的 HTML——当然找不到主页面的元素。

修复：`tests/conftest.py` 加一个 autouse fixture，每个测试执行前清掉这两个变量。

**为什么用 fixture 而不是在模块顶层直接删？**
模块顶层的代码在**导入阶段**执行，而 dotenv 的加载发生在
`src.web.app` 被导入的时候，两者顺序无法保证；
fixture 在**测试执行阶段**运行，此时所有导入都已完成，清理一定有效。

**教训**：配置注入会污染测试环境，而且报错位置和真实原因隔了好几层。
这类问题的信号是「报错信息和现象对不上」——看到这种信号就该往环境配置想。

### 部署结果

| 项目 | 值 |
|---|---|
| 线上地址 | https://english-windish.app.workbuddy.host/ |
| 托管方式 | 单端口 HTTP 服务，依赖在沙箱内安装 |
| 数据存储 | SQLite 随项目打包，无需外部数据库 |
| 访问保护 | 密码 + 每日 30 次上限 |

**实测验证**（全部针对线上地址）：

- 未登录访问首页 → 303 跳登录页；访问 API → 401
- 错误密码 → 跳回 `/login?e=1`；正确密码 → 下发凭证并放行
- 伪造 cookie → 被拒
- `/api/health` 返回 `access_protection: true`、`daily_limit: 30`、`problems: []`
  （最后一项为空说明 API 密钥配置完整）
- **端到端真实分析**：输入 `resilient` → 2 秒返回完整词卡
  （英/美音标、词根词缀、词源、分组释义、搭配例句、易混淆词对比）
