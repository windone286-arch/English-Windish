...
## 2026-09-19 · 阶段 2.6：三处细节修复

### 背景

功能做完不等于做对。用户实测后提了三条，都是「用起来别扭」而不是「功能缺失」：

1. 文本输入时，进度条第一步写着「识别图片文字」——可文本输入根本不经过这一步
2. 只输入一个单词，结果页签却叫「全文翻译」
3. 历史记录抽屉一直挂在页面右侧，点关闭也关不掉

这三条有个共同点：**代码没报错，测试全过，但用户一看就觉得不对。**
自动测试能拦住「程序崩溃」，拦不住「说法和事实不符」。

### 修复记录

**1. 进度阶段：图片和文本必须各有一套**

原因：两者共用同一份 `STAGES` 常量。图片链路 4 步，文本只有 3 步，
于是文本分析的进度条凭空多出一个永远不会执行的步骤。

修复：拆成 `IMAGE_STAGES` / `TEXT_STAGES`，`_build_event_stream()` 显式接收
`stages` 参数，由调用方传入。**不做模块级常量兜底**——兜底会把「传错了」
变成一个静默错误，而这正是当初出问题的原因。

加了两条测试锁住：文本阶段不含「识别图片文字」；
以及 `TEXT_STAGES == IMAGE_STAGES[1:]`，防止将来两套定义各自漂移。

**2. 文案要跟着输入粒度走**

「全文翻译」对句子成立，对单词不成立。输入 `accommodate`，
返回的是「容纳；适应；提供住宿；迁就」——这是**词义**，不是译文。

判断粒度看原文本的形态，不看模型输出：词数不超过 5 且没有句子标点，
就算单词或词组；否则算句子或段落。模型的 `structure` 字段其实也会说明
「仅为词条」，但那是模型的判断，可能不准；「有没有句号、一共几个词」
是客观事实，更可靠。

连带处理两处：
- 概览栏对单词输入不再显示「句子 0 / 语法点 0」——两个恒为 0 的数字只会让人以为出错
- 语法分栏对单词输入不渲染空句子卡片，直接说明「没有句子结构可分析」

**3. `[hidden]` 被 CSS 覆盖 —— 这次最隐蔽的一个**

现象：历史抽屉关不掉。

根因不在 JS，`closeDrawer()` 一直执行正常。问题出在 CSS：

```css
.drawer { display: flex; }   /* 这一条盖掉了浏览器的 [hidden] */
```

浏览器默认的 `[hidden] { display: none }` 属于**优先级最低的用户代理样式**，
只要作者样式表里给同一元素写了 `display`，就会把它覆盖。于是
`el.hidden = true` 变成空操作，元素纹丝不动。

修复：显式声明 `[hidden] { display: none !important; }`。
本项目所有面板的显隐都依赖 hidden 属性，这条规则一旦丢失整个界面都会坏，
所以补了测试守着它。

**教训**：用 `hidden` 属性控制显隐时，只要作者样式表给该元素写过 `display`，
就必须自己补这条规则，不能指望浏览器默认行为。

### 今日产出

- [x] `src/pipeline.py` —— 拆分 `IMAGE_STAGES` / `TEXT_STAGES`
- [x] `src/web/app.py` —— `_build_event_stream()` 接收 stages；健康检查分别返回两套
- [x] `src/web/static/style.css` —— 补 `[hidden]` 规则（关键修复）
- [x] `src/web/static/app.js` —— `detectInputKind()`，页签文案、概览栏、语法栏按粒度适配
- [x] `src/web/static/index.html` —— 页签标题加可改写的 `<span>`
- [x] `tests/test_web.py` / `tests/test_core.py` —— 新增 8 个测试
- [x] **97 个测试全部通过**（原 89 + 新增 8）

**实测验证**：
- `/api/health` 分别返回 `image_stages`（4 步）与 `text_stages`（3 步）
- 文本分析 `ambiguous` → start 事件 `total=3`，阶段列表不含「识别图片文字」
- 粒度判断逻辑用 11 个案例逐一验证（单词 / 词组 / 句子三个分支均正确）
